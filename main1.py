...
def task10(df0):
    ...
    df_f = df_fem.groupby(['Name']).sum()
    df_m = df_mal.groupby(['Name']).sum()

    df_test=pd.merge(df_f, df_m, left_index=True, right_index=True)

    df_test = df_test.rename(columns={'Number_x': 'Fem_number', 'Number_y': 'Mal_number'})
    df_test['ratio'] = df_test['Fem_number']/df_test['Mal_number']
    df_test['diff'] = df_test['Fem_number'] - df_test['Mal_number']

    #zblizona liczebnosc, ratio ~1
    df_test2 = df_test[df_test['ratio']>0.96]
    df_test2 = df_test2[df_test2['ratio'] < 1.04]
    print("Zad 10: ")
    print("Imiona o podobnej liczebnosci dla obu plci")
    list_of_names = df_test2.index
    for i in list_of_names:
        print(i)

    #jako nieznaczna przewage przyjalem 2
    df_test3 = df_test[df_test['diff']>-3]
    df_test3 = df_test3[df_test3['diff'] <0]

    print("Imiona o niewielkiej przewadze imion męskich:")
    list_of_names2 = df_test3.index
